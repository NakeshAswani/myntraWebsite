    ...
    <div>
      ...
      <div className={`grid ${(sideBar) ? "grid-cols-[200px_auto]" : "grid-cols-[100px_auto]"} pt-[84px]`}>
        ...
        <div className='w-full font-mono'>
          {/* <Location /> */}
          <div className='p-4'>
            <h1 className='w-max text-4xl font-bold mb-6 text-[white]   leading-[2.8rem]'>Add Slider</h1>
            <form className='w-8/12' onSubmit={addSlider}>
              <div className='mb-6'>
                <label className='block text-[18px] font-medium text-[white] mb-1'>Slider Number <span className='text-[red]'>*</span></label>
                <div className='w-full  bg-opacity-50 p-[2px_6px_6px_2px] rounded-lg'>
                  <input type="text" className='w-full bg-[white] rounded-md p-1 outline-offset-[-999px]' value={formData?.sliderNumber} name='sliderNumber' onChange={changeValue} />
                </div>
              </div>
              <div className='mb-6'>
                <label className='block text-[18px] font-medium mb-1 text-[white]'>Slider Name <span className='text-[red]'>*</span></label>
                <div className='w-full  bg-opacity-50 p-[2px_6px_6px_2px] rounded-lg'>
                  <input type="text" className='w-full bg-[white] rounded-md p-1 outline-offset-[-999px]' value={formData?.sliderName} name='sliderName' onChange={changeValue} />
                </div>
              </div>
              <div className='mb-6'>
                <label className='block text-[18px] font-medium mb-1 text-[white] '>Slider Description <span className='text-[red]'>*</span></label>
                <div className='w-full h-[120px]  bg-opacity-50 p-[2px_6px_6px_2px] rounded-lg'>
                  <textarea className='w-full h-full bg-[white] rounded-md p-1 outline-offset-[-999px] resize-none' value={formData?.sliderDescription} name='sliderDescription' onChange={changeValue}></textarea>
                </div>
              </div>
              <div className='mb-6'>
                <label className='block text-[18px] font-medium mb-1 text-[white]'>Slider Image <span className='text-[red]'>*</span></label>
                <div className='w-full  bg-opacity-50 p-[2px_6px_6px_2px] rounded-lg'>
                  <input type="file" className='file_upload w-full bg-[white] rounded-md p-1' name='sliderImage' />
                </div>
              </div>
